...
class Mail():
    ...
    def __init__(self):
        self.SMB_USER = 'reallytek.com\\dongming.lu'
        self.SMB_PASSWORD = 'Ludongming009'
    ...
    def getListDirBySmb(self,target_path,MODE='LS',file_path='',file_name=''):
        target_path = target_path
        MODE = MODE
        file_path = file_path
        file_name = file_name
        half_path = target_path.split('/',3)[3]
        share_dir = target_path.split('/',3)[2]
        if MODE=='LS':
            cmd = '"cd %s;ls"'%half_path
            smb = SmbClient.SmbClient(share_dir,self.SMB_USER,self.SMB_PASSWORD)
            result = smb.runCmd(cmd)
            return result
        if MODE=='GET':
            log_path = '%s/log.txt'%self.TEMPFILE
            print 'tempfile log :%s'%log_path
            if os.path.isfile(log_path):
                os.remove(log_path)
            cmd = '"cd %s;get log.txt %s"'%(half_path,log_path)
            smb = SmbClient.SmbClient(share_dir,self.SMB_USER,self.SMB_PASSWORD)
            smb.runCmd(cmd)
            if os.path.isfile(log_path):
                print 'get log.txt Succeed'
                return log_path
        if MODE=='PUT':
           os.chdir(file_path)
           cmd = '"cd %s;put %s"'%(half_path,file_name)
           smb = SmbClient.SmbClient(share_dir,self.SMB_USER,self.SMB_PASSWORD)
           result = smb.runCmd(cmd)
           os.chdir(self.cwd)
           print result
           if 'putting file' in result:
               print 'excel upload succeed'

    # get parameter from version_dir 
    def getInfoBySmb(self,path='',release_for=''):
        if path.strip()=='':
            result = self.getListDirBySmb(self.base_dir)
            self.date_dir = self.getDirName(result)
        ...
        print 'The target_dir is : %s'%self.win_dir
        version_info = self.getListDirBySmb(self.target_dir)
        patt_version = '^%s.*.zip'%self.version_base
        patt_xml = '^%s(?!.*-CLEAN-SNAPSHOT-).*.xml'%self.version_base
        for value in version_info.split('\n'):
            for one in value.split(' '):
                version = self.matchInfo(patt_version,one)
                if version is not None:
                    self.version_number = version[:-4]
                xml = self.matchInfo(patt_xml,one)
                if xml is not None:
                    self.snap = xml
        print 'version number : %s \n xml_name: %s'%(self.version_number,self.snap)
        self.change_log = self.getListDirBySmb(self.target_dir,'GET')
        self.log_list=self.getLogList(self.change_log)
        self.release_for=release_for
    
    def matchInfo(self,patt,string):
        result = re.match(patt,string)
        if result is not None:
            result = result.group()
            print '匹配到结果：%s'%result
            return result
 
    def getDirName(self,result):
        result = result
        line = result.split('\n')
        for i in range(len(line)-6):
            print '%d:%s'%(i+1,line[i+4])
        number_dir = int(raw_input("choose target_dir for send mail: "))
        print '切割出来的文件夹内容:%s'%(line[number_dir+3].split(' '))
        date_dir = line[number_dir+3].split(' ')[2]
        return date_dir
    ...
    def sendMail(self,to_mail):
        ...
        server.quit()
        #save Excel
        excel_file = '%s/ReleaseNote.xls'%self.TEMPFILE
        self.saveExcel(excel_file)
        if os.path.isfile(excel_file):
            self.getListDirBySmb(self.target_dir,'PUT',self.TEMPFILE,'ReleaseNote.xls')
...
if __name__=='__main__':
    ...
    M.getInfoBySmb(base_Dir,release_For)
    if len(sys.argv)==1:
