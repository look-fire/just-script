...
function get_dir_parameters(){
    log_info "get Dir parameters"
    DIR_TIME_ONE=$(echo "$(python -c "import commands; print commands.getoutput('ls -t "$ARCHIVE_BASE_DIR"/"$ARCHIVE_PRODUCT_DIR_RELATIVE"').split('\n')[0]")")
    DIR_TIME_TWO=$(echo "$(python -c "import commands,re ; m=commands.getoutput('ls -t "$ARCHIVE_BASE_DIR"/"$ARCHIVE_PRODUCT_DIR_RELATIVE"').split('\n'); 
for i in range(1,len(m)):
    an = re.search('^.*[_error,_ENG]$', m[i])
    if  an :
        continue
    else:
        print m[i]
        break")")
    TEST_INCREMENTAL_FROM="$ARCHIVE_BASE_DIR"/"$ARCHIVE_PRODUCT_DIR_RELATIVE"/"$DIR_TIME_ONE"/full*
    TEST_INPUT_TARGET_FILE="$ARCHIVE_BASE_DIR"/"$ARCHIVE_PRODUCT_DIR_RELATIVE"/"$DIR_TIME_TWO"/full*
    TEST_OTA_TOOLS="$ARCHIVE_BASE_DIR"/"$ARCHIVE_PRODUCT_DIR_RELATIVE"/"$DIR_TIME_ONE"/OTA_TOOLS.zip
}
...
function get_otatest_automatic(){
    ...
    INCREMENTAL_FROM=$(echo ${TEST_INCREMENTAL_FROM})
    INPUT_TARGET_FILE=$(echo ${TEST_INPUT_TARGET_FILE})
    
    OTA_TOOLS=$(echo "$TEST_OTA_TOOLS"| sed 's#mnt#\/192.168.1.75#g')
    INCREMENTAL_FROM_TEST=$(echo "$INCREMENTAL_FROM" | sed 's#mnt#\/192.168.1.75#g')
    INPUT_TARGET_FILE_TEST=$(echo "$INPUT_TARGET_FILE" | sed 's#mnt#\/192.168.1.75#g')
    
    FROM_OTA_NAME=$(get_base_ota_name "${INCREMENTAL_FROM_TEST}")
    TARGET_OTA_NAME=$(get_base_ota_name "${INPUT_TARGET_FILE_TEST}")
